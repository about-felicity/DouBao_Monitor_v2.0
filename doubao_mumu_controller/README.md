...
- `打开豆包MuMu控制面板.bat`：推荐使用。管理大量问题、每题独立重复次数、立即运行和 Windows 定时任务。
- `一键启动_手机发送并网页抓取.bat`：不打开面板，直接运行 `questions.txt`。自动识别 MuMu、核对手机/网页账号、在 MuMu 发送，再从网页抓取同一条新回答并写入 monitor 原有结果文件。
- `open_doubao_mumu_loop.bat`：只控制 MuMu 发送和读取回答，不运行网页抓取。

## 控制面板

双击 `打开豆包MuMu控制面板.bat`。面板支持：

- 添加、编辑、删除和 TXT 批量导入问题；
- 每个问题单独设置重复次数；
- 交叉提问或顺序提问；
- 设置每轮等待、超时、重试和随机冷却；
- 检测 MuMu 实例、App 账号 UID 和网页账号是否一致；
- 自动并行管理全部已启动的 MuMu；“实例”留空表示全部，也可填写
  `0,1,3` 只选择部分实例；
- 每个 MuMu 实例对应一个独立 Chrome 用户目录、调试端口和豆包账号，
  多账号之间不会串会话；
- 立即执行整批任务并显示实时日志；
...
1. 用 `MuMuManager.exe` 获得目标实例的真实 ADB 地址。
2. 临时执行 `adb root`，只读
   `/data/user/0/com.larus.nova/databases/account_db`。
3. 从 `login_info` 表按登录时间读取最新记录的数字 `uid` 和昵称。
4. 立即执行 `adb unroot` 并恢复连接。
...
1. 启动 MuMu，并在 MuMu 豆包 App 中登录账号。
2. 编辑 `questions.txt`，一行一个问题。
3. 双击 `一键启动_手机发送并网页抓取.bat`。

脚本会自动完成：

1. 通过 `MuMuManager.exe` 发现已启动实例和真实 ADB 端口。
2. 临时启用 MuMu ADB root，只读豆包 `account_db` 中最后登录账号的数字 UID，随后立即恢复 unroot。
3. 为每个 MuMu 实例分配独立 Chrome 调试端口，要求该网页 UID 与对应
   MuMu UID 完全一致。
4. 若找不到匹配网页，会按“账号 UID + MuMu 实例”建立独立 Chrome
   配置并打开豆包；每个账号第一次需要人工登录一次。全部匹配后才允许开始。
5. 在网页端记录发送前的会话集合。
6. 控制 MuMu 新建对话、输入、校验、发送并等待回答稳定。
...
- 每个 MuMu 实例使用独占文件锁；多实例可以并行，同一实例不会被重复抢控。
...
- 首次在新电脑运行需要 Python、Chrome、MuMu，以及可用的 Appium（当前脚本也能使用影刀附带的 Appium）。
...
- `doubao_mumu_web_diagnostics\`：异常时的 MuMu XML、截图和说明。
...
1. 安装 Google Chrome 和 MuMu；完整部署包已经包含便携 Python、
   Appium 和 ADB。
2. 启动 MuMu，在豆包 App 登录并保持在豆包页面。
3. 解压生成的 ZIP，双击根目录的 `一键启动远端采集.bat`。
4. 程序会发现所有已启动 MuMu，并为每台打开一个专用 Chrome。分别登录
