...
- 每次发送前检查豆包 Java 堆；默认达到 190MB 时主动重启 App，避免撞到
  逍遥 Android 9 的 256MB 堆上限。检测到进程消失、“屡次停止运行”弹窗或
  “豆包没有响应（ANR）”时，也会自动关闭并恢复，当前账号数据不会被清除。
- 每轮网页抓取、本地保存和回传入队全部成功后，默认停止豆包进程释放整块
  Java 堆，利用轮间冷却时间保持关闭；下一轮自动启动。这个操作不是“清除数据”，
  不会删除账号。仅调试时可传 `--keep-app-running-between-rounds` 禁用。
- 已经启动、无法中途重启加载新代码的长任务，可另行运行
  `doubao_mumu_watchdog.py`。它只在 Android 明确报告 ANR 或应用崩溃时恢复豆包，
  不会清除账号，也不会干预正常回答页面；旧任务还可加 `--restart-missing`
  在豆包进程意外退出时自动拉起。
- 同一实例若在 3 分钟内连续发生 3 次 ANR/崩溃，会升级为清理豆包的
  `cache` 与 `code_cache` 后重启；数据库、登录信息和对话数据不会删除。
- 逍遥 Android 9 的系统异常框不会随 `force-stop` 自动消失；恢复流程会先按
  当前分辨率点击“关闭应用”所在行，再重启豆包，避免对同一个残留弹窗反复恢复。
- 手机端已确认发送后，网页抓取失败不会再次发送同一问题。
- 若手机端持续转圈且网页超过 `--sync-timeout` 仍未出现新会话，会判定本次生成
  已卡死，重启豆包并在新对话重试当前问题，避免永久停在同一轮。
- 长任务异常退出后可用 `--start-round N` 保留前 N 轮成功结果并从第 N+1 轮续跑。
- `--max-round-retries 0` 为持续恢复，默认不会因单次异常退出。
- 只有人工关闭窗口或按 `Ctrl+C` 才会安全停止。
- Appium 降为警告级日志，整批任务日志限制为 8MB × 3 个备份，避免长期运行
  生成数百 MB 日志。需要调整内存保护阈值时可设置环境变量
