...
def android_sdk_root_for_adb(adb_path: str | Path) -> Path:
    ...
    if adb.parent.name.casefold() == "platform-tools":
        ...
        if (root / "platform-tools" / "adb.exe").exists():
            return root
...
def resolve_java_home() -> Path:
    ...
    candidates = ([Path(configured)] if configured else []) + JAVA_HOME_CANDIDATES
    for candidate in candidates:
        if (candidate / "bin" / "java.exe").exists():
            return candidate.resolve()
        if candidate.exists():
            nested = next(candidate.glob("*/bin/java.exe"), None)
    ...
    for value in located:
        java = Path(value.strip())
        if java.exists() and java.parent.name.casefold() == "bin":
            return java.parent.parent.resolve()
    raise AutomationError(
        "找不到 Java 运行环境；Appium 需要 JAVA_HOME。"
    )


def now_text() -> str:
    return datetime.now(BEIJING_TZ).isoformat(sep=" ", timespec="seconds")
        ...
        for serial in candidates:
            ...
            for _attempt in range(3):
                ...
                if state.returncode == 0 and state.stdout.strip() == "device":
                    # A successful shell call filters out MuMu's short-lived
                    # false-ready state after adb root/unroot.
                    ...
                    if model_result.returncode == 0:
                        ...
                        return serial
                state_text = (
                    f"{state.stdout if state else ''} "
                    f"{state.stderr if state else ''}"
                ).lower()
                if "offline" in state_text and ":" in serial:
                    self._run(
                        ["disconnect", serial],
                        serial="",
                        timeout=8,
...
class AppiumClient:
    ...
    def _start_server(self) -> None:
        ...
        listen_port = parsed_url.port or 4723
        base_path = (parsed_url.path or "/wd/hub").rstrip("/") or "/wd/hub"
        ...
        sdk_root = android_sdk_root_for_adb(self.adb.adb_path)
        java_home = resolve_java_home()
        appium_env = os.environ.copy()
        appium_env["ANDROID_HOME"] = str(sdk_root)
        appium_env["ANDROID_SDK_ROOT"] = str(sdk_root)
        appium_env["JAVA_HOME"] = str(java_home)
        platform_tools = str(sdk_root / "platform-tools")
        java_bin = str(java_home / "bin")
        current_path = str(appium_env.get("PATH") or "")
        prepend = [
            value
            for value in (platform_tools, java_bin)
            if value.casefold() not in current_path.casefold()
        ]
        if prepend:
            appium_env["PATH"] = os.pathsep.join(prepend + [current_path])
        ...
        self.logger.warning(
            "Appium 未运行，已自动启动；Android SDK=%s；Java=%s；日志：%s",
            sdk_root,
            java_home,
            log_path,
        )
...
def has_text_input_control(root: ET.Element) -> bool:
    """Accept both Doubao chat composer variants."""
    return has_id(root, INPUT_ID) or has_id(root, INPUT_TOGGLE_ID)
...
def find_new_chat_node(root: ET.Element) -> ET.Element | None:
    ...
    for node in root.iter():
        ...
        if (
            description in NEW_CHAT_DESCRIPTIONS
            or text in NEW_CHAT_DESCRIPTIONS
        ) and node.attrib.get("clickable") == "true":
            return node
    # Some Doubao builds expose neither text nor accessibility description,
    # but keep a clickable top-right compose/plus control.
    candidates = []
    for node in root.iter():
        ...
        if (
            ...
            candidates.append((bounds[0], node))
    ...
    def create_new_chat(self) -> None:
        ...
        for recovery_round in range(1, 7):
            ...
            if state == "list":
                ...
                if new_chat is None:
                    raise AutomationError(
                        "对话列表找不到右上角新建入口（已兼容编辑图标、加号和无障碍文本）。"
                    )
                try:
                    if new_chat.attrib.get("resource-id"):
                        self.appium.click_id(
                        ...
                        bounds = node_bounds(new_chat)
                        if bounds is None:
                            raise AutomationError("新建入口没有可点击坐标。")
                        left, top, right, bottom = bounds
                        self.adb.shell(
                            "input",
                            "tap",
                ...
                # Current Doubao versions open a blank chat directly from the
                # compose button.  Treat that chat page as success instead of
                # backing out to the list again.
                if (
                    page_name(destination) == "chat"
                    and has_text_input_control(destination)
                ):
                    self.logger.info("已通过对话列表直接创建新对话")
                    return
                continue
