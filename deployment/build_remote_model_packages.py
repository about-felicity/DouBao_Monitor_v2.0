...
MODELS = {
    "deepseek": ("DeepSeek", ROOT / "collectors" / "deepseek" / "legacy_app"),
    "yuanbao": ("腾讯元宝", ROOT / "collectors" / "yuanbao"),
    "wenxin": ("文心", ROOT / "collectors" / "wenxin"),
}
EXCLUDED_NAMES = {
    "__pycache__",
    "chrome_profile",
...
def build_package(model: str, pairing: dict, output_root: Path, stamp: str) -> tuple[Path, Path]:
    ...
    copy_source_tree(ROOT / "transport", package_root / "transport")
    copy_source_tree(ROOT / "dashboard" / "backend", package_root / "dashboard" / "backend")
    copy_source_tree(ROOT / "collectors" / "plugins" / model, package_root / "collectors" / "plugins" / model)
    copy_source_tree(model_source, package_root / model_source.relative_to(ROOT))
    for init_file in (
        ROOT / "collectors" / "__init__.py",
        ROOT / "collectors" / "plugins" / "__init__.py",
        ROOT / "dashboard" / "__init__.py",
    ):
        destination = package_root / init_file.relative_to(ROOT)
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(init_file, destination)
    shutil.copy2(ROOT / "deployment" / "remote_worker_requirements.txt", package_root / "remote_worker_requirements.txt")
    ...
    launcher_content = (
        ...
        f"%PY_CMD% transport\\remote_worker.py --model {model} --preflight\r\n"
        "if errorlevel 1 (echo Preflight failed & pause & exit /b 1)\r\n"
        f"%PY_CMD% dashboard\\backend\\control_panel.py --model {model}\r\n"
