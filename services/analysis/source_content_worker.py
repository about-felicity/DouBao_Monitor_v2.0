...
DEEPSEEK_RESULTS = BASE_DIR / "collectors" / "deepseek" / "chrome" / "data" / "deepseek_results.jsonl"
YUANBAO_RESULTS = BASE_DIR / "collectors" / "yuanbao" / "yuanbao_results.jsonl"
WENXIN_RESULTS = BASE_DIR / "collectors" / "wenxin" / "wenxin_results.jsonl"
...
def full_video_title(raw_html, fallback=""):
    """Return the richest caption/title exposed by a rendered video page."""
    candidates = [normalize_text(fallback)]
    try:
        root = html.fromstring(raw_html)
        for xpath in (
            "//title/text()",
            ...
            candidates.extend(normalize_text(value) for value in root.xpath(xpath))
    except (etree.ParserError, ValueError):
        pass
    candidates.extend(embedded_descriptions(raw_html))
    candidates = [value for value in candidates if 4 <= len(value) <= 20000]
    return max(candidates, key=len, default=normalize_text(fallback))


def douyin_visible_caption(visible_text, fallback=""):
    """Extract the complete caption displayed above Douyin's publish time.

    Douyin frequently leaves title/description meta tags empty.  The complete
    caption is nevertheless rendered in the page body, immediately before the
    expand control and publish-time row.
    """
    lines = [normalize_text(line) for line in str(visible_text or "").splitlines()]
    lines = [line for line in lines if line]
    for index, line in enumerate(lines):
        if line.startswith("发布时间"):
            nearby = [
                value for value in lines[max(0, index - 4):index]
                if value not in {"展开", "收起", "详情"}
            ]
            if nearby:
                return max(nearby, key=len)
    candidates = [
        line for line in lines
        if 30 <= len(line) <= 20000
        and line not in {"视频数据加载中", "暂时没有更多评论"}
    ]
    return max(candidates, key=len, default=normalize_text(fallback))
...
def needs_hidden_douyin_title(url, title):
    """Detect a Douyin card whose visible caption is probably truncated."""
    ...
    return (
        ...
        and (len(cleaned) >= 90 or bool(re.search(r"(?:[.…#]|[A-Za-z])$", cleaned)))
    )
...
def fetch_douyin_with_hidden_browser(url):
    """Read a Douyin detail page without waiting for video resources to finish.

    Some slideshow posts are shared as ``/share/video`` but only expose their
    full caption on the ``/note/<id>`` view.  Try that equivalent view when the
    normal detail page remains on the loading shell.
    """
    from patchright.sync_api import sync_playwright

    candidates = [url]
    match = re.search(r"/(?:share/video|video|note)/(\d+)", url)
    if match:
        item_id = match.group(1)
        candidates.extend([
            "https://www.douyin.com/note/" + item_id,
            "https://www.douyin.com/video/" + item_id,
        ])

    last_error = None
    with DYNAMIC_FETCH_LOCK, sync_playwright() as playwright:
        ...
        try:
            ...
            for candidate in dict.fromkeys(candidates):
                try:
                    ...
                    caption = douyin_visible_caption(visible_text)
                    if len(caption) >= 30 and caption != "视频数据加载中":
                        raw = page.content().encode("utf-8", errors="ignore")[:MAX_BYTES]
                        return {
                            "status_code": int(response.status if response else 200),
                            "final_url": str(page.url or candidate),
                            "content_type": "text/html; charset=utf-8",
                            "raw": raw,
                            "visible_text": visible_text,
                            "page_caption": caption,
                            "transport": "patchright_hidden_douyin",
                        }
                except Exception as exc:
                    last_error = exc
            if last_error:
                raise last_error
            raise RuntimeError("Douyin page stayed on its loading shell")
        finally:
            browser.close()
...
def fetch_one(url, previous_attempts=0, source_title=""):
    ...
    try:
        with host_lock:
            force_hidden_video = needs_hidden_douyin_title(url, source_title)
            if force_hidden_video:
                # The result-card caption is routinely clamped at ~100 chars.
                # Render the actual Douyin page in an isolated headless Chrome
                # and read its metadata instead of guessing from the snippet.
                response = fetch_douyin_with_hidden_browser(url)
            else:
                try:
        ...
        if force_hidden_video:
            title = normalize_text(
                response.get("page_caption")
                or full_video_title(decoded, title or source_title)
                or source_title
            )
            visible_text = normalize_text(response.get("visible_text"))
            if len(visible_text) > len(content_text):
                content_text = visible_text[:MAX_TEXT_CHARS]
            method = "douyin_hidden_browser_metadata"
            quality = "high"
        if (
        ...
        if len(content_text) < MIN_CONTENT_CHARS and not (
            force_hidden_video and len(title) >= 30
        ):
            return {
...
def database_source_buckets(rows):
    """Deduplicate a streamed database source scan without a PostgreSQL sort."""
    buckets = {"deepseek": {}, "yuanbao": {}, "wenxin": {}}
    latest = {}
    counts = Counter()
    for row in rows:
        ...
        rank = (day, run_no, int(row.get("source_index") or 0))
        if rank >= latest.get(key, ("", -1, -1)):
            latest[key] = rank
    ...
    for (model_id, url), frequency in counts.items():
        run_no, title, day, _old_frequency = buckets[model_id][url]
...
def _load_database_sources(days=7):
    ...
    """
    empty = {"deepseek": {}, "yuanbao": {}, "wenxin": {}}
    if not monitor_database.enabled():
        return empty
    try:
        monitor_database.ensure_schema()
        with monitor_database.connection() as connection:
            # The former COUNT/ROW_NUMBER window query sorted every recent
            # source in PostgreSQL and filled C:\ with multi-gigabyte temp
            # files.  Stream the narrow rows and do the small URL map in
            # Python instead; this query has no sort or temp-file phase.
            with connection.cursor(name="content_source_scan") as cursor:
                cursor.itersize = 2_000
                cursor.execute(
                    "SELECT r.model_id,r.sequence_no,r.day,s.source_index,"
                    "COALESCE(NULLIF(btrim(s.url),''),NULLIF(btrim(s.payload->>'url'),''),"
                    "NULLIF(btrim(s.payload->>'href'),'')) AS source_url,"
                    "COALESCE(NULLIF(s.title,''),s.payload->>'title','') AS source_title "
                ...
                return database_source_buckets(cursor)
    except Exception as exc:
...
def collect_urls():
    ...
    source_files = () if database_enabled else (
        DEEPSEEK_RESULTS, YUANBAO_RESULTS, WENXIN_RESULTS
    )
    ...
    for index in range(max_len):
        for model_id in ("deepseek", "yuanbao", "wenxin"):
...
def article_urls(urls):
    """Select articles plus long Douyin cards that require browser resolution."""
    selected = []
    for item in urls:
        row = {"href": item["url"], "title": item.get("title", "")}
        source_type, _media, _host, _note = dashboard.source_for(row, {}, {})
        if "文章" in source_type or needs_hidden_douyin_title(item["url"], item.get("title", "")):
...
def fair_pending_selection(items, limit):
    ...
    order = [name for name in ("wenxin", "yuanbao", "deepseek", "other") if groups[name]]
    selected = []
    index = 0
...
def pid_is_running(pid):
    ...
    try:
        ...
        if ":" in owner:
            try:
                start_ticks = Path(f"/proc/{pid}/stat").read_text(
                    encoding="ascii"
                ).split()[21]
                return owner == f"{pid}:{start_ticks}"
            except (OSError, IndexError):
                return False
        return not (Path("/.dockerenv").exists() and pid == 1)
    except OSError:
        return False


def process_lock_token():
    pid = os.getpid()
    if os.name != "nt":
        try:
            start_ticks = Path(f"/proc/{pid}/stat").read_text(
...
def acquire_lock():
    ...
    try:
        ...
        os.write(descriptor, process_lock_token().encode("ascii"))
        return True
...
def release_lock():
    try:
        if LOCK_PATH.exists() and LOCK_PATH.read_text(encoding="ascii", errors="ignore").strip() == process_lock_token():
            LOCK_PATH.unlink()
    except Exception:
        pass
...
def process_batch(index, connection, urls, brands, vocab_hash, limit):
    ...
    with ThreadPoolExecutor(max_workers=WORKERS, thread_name_prefix="source-content") as pool:
        ...
        for future in as_completed(futures):
            ...
            if result.get("status") == "ok":
                ok += 1
                if needs_hidden_douyin_title(url, item.get("title", "")):
                    try:
                        repair = monitor_database.apply_browser_source_title(
                            url, result.get("title", "")
                        )
                        log("douyin hidden-title repair url=%s result=%s" % (url, repair))
                    except Exception as repair_exc:
                        log("douyin hidden-title repair failed url=%s error=%s" % (
                            url, repr(repair_exc)[:240],
                        ))
...
def main():
    ...
    parser.add_argument("--model", choices=("deepseek", "yuanbao", "wenxin"))
