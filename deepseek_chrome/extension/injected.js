(function () {
  "use strict";
  if (window.__deepseekMonitorNetworkHookInstalled) return;
  window.__deepseekMonitorNetworkHookInstalled = true;
  ...
  function emit(items, requestUrl) {
    ...
    window.dispatchEvent(new CustomEvent("deepseek-monitor:sources", {
  ...
  XMLHttpRequest.prototype.open = function (method, url, ...rest) {
    this.__deepseekMonitorUrl = url;
    return originalOpen.call(this, method, url, ...rest);
  };
  XMLHttpRequest.prototype.send = function (...args) {
    this.addEventListener("load", function () {
      try {
        if (typeof this.responseText === "string") scanText(this.responseText, this.responseURL || this.__deepseekMonitorUrl);
      } catch (_) {}
    }, { once: true });
    return originalSend.apply(this, args);
  };

  window.addEventListener("deepseek-monitor:reset", () => {
    window.dispatchEvent(new CustomEvent("deepseek-monitor:sources", { detail: { reset: true, items: [] } }));
  });
})();
