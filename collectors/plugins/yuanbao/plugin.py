...
class Plugin(ModelPlugin):
    ...
    questions = ROOT / "collectors" / "yuanbao" / "product.txt"
    runner = ROOT / "collectors" / "yuanbao" / "yuanbao_loop.py"
    results = ROOT / "collectors" / "yuanbao" / "yuanbao_results.jsonl"
    collector_results = ROOT / "runtime" / "remote_workers" / "yuanbao_collector_results.jsonl"
    dashboard = ROOT / "dashboard" / "frontend" / "public" / "data" / "dashboard.json"
    builder = ROOT / "collectors" / "yuanbao" / "build_dashboard_data.py"
    execution = "remote"
    ...
    def account_check(self) -> dict[str, Any]:
        from collectors.yuanbao.bowser import ensure_yuanbao_chrome, yuanbao_web_identity
        from collectors.yuanbao.controller import YuanbaoController
        from collectors.yuanbao.yuanbao_loop import (
            BROWSER_MAP_PATH,
        ...
        for order, device in enumerate(devices):
            ...
            port = 9222 + order
            profile = (
                ROOT / "collectors" / "yuanbao" / "chrome_profile_auto"
                if len(devices) == 1
                else ROOT / "collectors" / "yuanbao" / "chrome_profiles" / safe_serial(serial)
            )
