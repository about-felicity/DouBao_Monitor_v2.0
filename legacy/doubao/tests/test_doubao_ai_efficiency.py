...
from collectors.doubao.core import save_doubao_refs as saver
from services.analysis import doubao_product_worker as product_worker
from services.analysis import doubao_source_worker as source_worker
...
class ProductAiEfficiencyTests(unittest.TestCase):
    ...
    def test_wenxin_opening_recommendation_keeps_latin_bath_oil_brand(self):
        answer = (
            "推荐HARNN玫瑰沐浴按摩油，它是泰国进口的SPA级沐浴精油。\n"
            "核心亮点✨\n成分与功效：以天然玫瑰植萃精油为核心。"
        )
        products = saver.extract_structured_product_headings(answer, "沐浴精油推荐")

        self.assertEqual(len(products), 1)
        self.assertEqual(products[0]["product_name"], "HARNN玫瑰沐浴按摩油")
        self.assertEqual(products[0]["brand_name"], "HARNN")
        self.assertEqual(
            len(saver.filter_products_for_question("沐浴精油推荐", products)), 1,
        )

    def test_quark_representative_product_row_is_split(self):
        answer = (
            "1. 追求极致温和与全能效果\n"
            "代表产品：麦致植萃染发剂、首迷植萃染发剂、梵玢 FBCY\n"
            "特点：无氨配方，适合敏感头皮。"
        )
        products = saver.extract_structured_product_headings(answer, "染发剂推荐")
        self.assertEqual(
            [item["product_name"] for item in products[:2]],
            ["麦致植萃染发剂", "首迷植萃染发剂"],
        )
    ...
    def test_brand_from_another_product_cannot_be_cross_assigned(self):
        products = saver.ground_product_brands(
            "先推荐白云山产品；更推荐科熙本二硫化硒去屑止痒洗发水。",
            [{
                "product_name": "科熙本二硫化硒去屑止痒洗发水",
                "brand_name": "白云山",
                "evidence": "更推荐科熙本二硫化硒去屑止痒洗发水",
            }],
        )
        self.assertEqual(products[0]["brand_name"], "")
        self.assertFalse(products[0]["brand_identified"])
    ...
    def test_grounding_accepts_literal_heading_with_adjacent_reason_evidence(self):
        products = saver.validate_grounded_ai_products(
            "1. 理然 造型打底蓬松水。定型自然，适合日常通勤。",
            [{
                "product_name": "理然 造型打底蓬松水",
                "brand_name": "理然",
                "evidence": "定型自然，适合日常通勤",
            }],
        )
        self.assertEqual(products[0]["brand_name"], "理然")
