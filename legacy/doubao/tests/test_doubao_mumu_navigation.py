...
from collectors.doubao.mumu.doubao_mumu_loop import (
    BACK_ID,
    CHAT_ROOT_ID,
    INPUT_ID,
    LIST_ID,
...
class NewChatNavigationTests(unittest.TestCase):
    ...
    def test_detects_android_anr_dialog(self):
        root = parse_xml(
            '<?xml version="1.0"?><hierarchy>'
            '<node text="豆包没有响应" />'
            '<node text="关闭应用" />'
    ...
    @patch("collectors.doubao.mumu.doubao_mumu_loop.time.monotonic", return_value=120)
    ...
    @patch("collectors.doubao.mumu.doubao_mumu_loop.time.sleep", return_value=None)
