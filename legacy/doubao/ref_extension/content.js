(function () {
  ...
  function extractReferences() {
    ...
    referenceAnchors().forEach((a) => {
      // The visible card text is often CSS-clamped to exactly 100 characters,
      // while textContent/title/aria-label still carries the owned-brand
      // hashtag at the end. Prefer the richest clean candidate.
      const title = [
        a.innerText,
        a.textContent,
        a.getAttribute("aria-label"),
        a.getAttribute("title")
      ].map(cleanText).sort((left, right) => right.length - left.length)[0] || "";
  ...
  async function expandAndExtract() {
    ...
    for (let i = 0; i < 8; i += 1) {
      ...
      if (rows.length >= expected) {
        if (!rows.some((item) => item.title.length >= 95 || /[.…#]$/.test(item.title))) return rows;
        try {
          const backendRows = await extractBackendReferences();
          const richer = new Map(backendRows.map((item) => [item.href, item]));
          return rows.map((item) => {
            const candidate = richer.get(item.href);
            return candidate && candidate.title.length > item.title.length
              ? { ...item, title: candidate.title, source: `${item.source} + doubao backend title` }
              : item;
          });
        } catch (_) {
          return rows;
