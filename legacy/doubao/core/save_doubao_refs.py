...
def strip_reference_prefix(text):
    ...
    header_pattern = re.compile(
        r"^\s*(?:#{1,6}\s*)?(?:"
        + "|".join(re.escape(item) for item in sorted(TRAILING_SECTION_HEADERS, key=len, reverse=True))
        + r")[\s:：—-]*$",
        re.I,
    )
    ...
    if not result and not found_trailing_section:
        return text
    return result
...
PRODUCT_CATEGORY_TERMS = {
    ...
    "护发素": ("护发素", "润发乳", "护发乳", "发膜"),
    "控油蓬松洗发水": ("洗发水", "洗发露", "洗发乳"),
    "沐浴精油": ("沐浴精油", "沐浴油", "沐浴按摩油", "沐浴精华油", "芳香沐浴露"),
    "眉毛增长液": (
        "眉毛", "密眉", "眉部", "眉睫", "生眉", "养眉", "米诺地尔", "多肽精华",
    ),
...
def ground_product_brands(answer_text, products):
    ...
    for item in products or []:
        ...
        # A brand occurring somewhere else in the same long answer does not
        # prove that it belongs to this product.  Ground the relationship in
        # the row itself (product name + quoted evidence) to prevent cross-row
        # swaps such as assigning 白云山 to a 科熙本 product.
        row_scope = _grounding_text(
            str(product.get("product_name") or "") + " "
            + str(product.get("evidence") or "")
        )
        identified = bool(brand and any(alias in row_scope for alias in aliases))
        if not identified:
            product["brand_name"] = ""
...
def validate_grounded_ai_products(answer_text, products):
    ...
    for item in products or []:
        ...
        if not candidate and grounded_alias:
            continue
        # The quoted evidence may be the adjacent reason rather than the list
        # heading that contains the literal product name.  The evidence itself
        # is already required to occur in the answer, so an exact product-name
        # occurrence anywhere in that same answer is equally grounded.
        if candidate and candidate in answer:
            continue
...
def infer_brand_from_product_name(product_name):
    ...
    # Imported brands are often joined directly to a Chinese series name, such
    # as HARNN玫瑰沐浴按摩油. Keep the leading Latin master brand rather than
    # absorbing the Chinese series adjective into the brand.
    latin_prefix = re.match(r"^([A-Za-z][A-Za-z0-9&.'-]{1,24})(?=[\u4e00-\u9fff])", text)
    if latin_prefix:
        return _normalize_brand(latin_prefix.group(1))
...
def extract_structured_product_headings(answer_text, question=""):
    ...
    normalized_question = str(question or "").strip().replace("推荐一款", "").replace("推荐", "")
    category_terms = PRODUCT_CATEGORY_TERMS.get(normalized_question, ())
    # Wenxin often uses one complete opening sentence instead of a numbered
    # product heading. Accept it only when the wording is explicitly positive,
    # belongs to the requested category and starts with a known brand or an
    # unambiguous Latin master brand.
    for original in lines:
        match = re.match(
            r"^(?:首推|优先推荐|推荐(?:一款)?|优先选择)\s*"
            r"(?!几款|一些)([^，,。；;！？!?]{2,60})",
            original,
        )
        if not match:
        ...
        if category_terms and not any(term in candidate for term in category_terms):
            continue
        folded_candidate = candidate.casefold()
        has_known_brand = any(
            str(brand).casefold() in folded_candidate
            for brand in known_brands if len(str(brand)) >= 2
        )
        has_latin_brand = bool(re.match(
            r"^[A-Za-z][A-Za-z0-9&.'-]{1,24}(?=[\u4e00-\u9fff])", candidate,
        ))
        if not (has_known_brand or has_latin_brand):
            continue
        key = re.sub(r"[^0-9a-z\u4e00-\u9fff]+", "", candidate, flags=re.I).casefold()
        if not key or key in seen:
            continue
        seen.add(key)
        brand = infer_brand_from_product_name(candidate)
        found.append({
            "product_name": normalize_known_product_alias(candidate),
            "brand_name": brand,
            "brand_identified": bool(brand),
    ...
    # Quark frequently groups several literal names on one ``代表产品`` row
    # instead of giving each product its own heading.  Split only that explicit
    # label; ordinary prose remains model-reviewed and cannot create products.
    for original in lines:
        representative = re.match(r"^代表产品\s*[：:]\s*(.+)$", original)
        if not representative:
            continue
        for raw_candidate in re.split(r"[、，,；;]", representative.group(1)):
            candidate = re.sub(r"^(?:例如|比如|如)\s*", "", raw_candidate).strip()
            ...
            if not has_known_brand and category_terms and not any(
                term in candidate for term in category_terms
            ):
                continue
            key = re.sub(r"[^0-9a-z\u4e00-\u9fff]+", "", candidate, flags=re.I).casefold()
            if not key or key in seen:
