...
MONITOR_DIR = BASE_DIR.parents[2]
DASHBOARD_SCRIPT = MONITOR_DIR / "dashboard" / "backend" / "server.py"
...
def environment_check() -> dict[str, object]:
    ...
    require_file(
        MONITOR_DIR / "collectors" / "doubao" / "core" / "run_doubao_latest_grab.py",
        "网页抓取程序",
    )
    require_file(
        MONITOR_DIR / "collectors" / "doubao" / "ref_extension" / "manifest.json",
        "内置网页抓取器",
    )
