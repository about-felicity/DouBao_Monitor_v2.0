...
from controller import AfuAppController, AfuWebCollector
...
from transport.sync import enqueue as enqueue_remote_result
...
def main() -> int:
    ...
    parser.add_argument("--chrome-port", type=int, default=9555)
    parser.add_argument("--wait", type=float, default=30)
    parser.add_argument("--random-wait", type=float, default=90)
    parser.add_argument("--timeout", type=int, default=180)
    parser.add_argument("--results", default=str(BASE_DIR / "afu_results.jsonl"))
    parser.add_argument("--state", default=str(BASE_DIR / "afu_state.json"))
    parser.add_argument("--log", default=str(BASE_DIR / "afu_loop.log"))
    ...
    app, web = AfuAppController(args.serial), AfuWebCollector(args.chrome_port)
    ...
    while not STOP and index < len(schedule):
        ...
        started = now()
        try:
            with device_session(args.serial, "蚂蚁阿福", timeout=args.timeout + 120, on_wait=log.info):
                previous = web.latest_reference()
                app.send(prompt)
                mobile = app.wait_for_answer(prompt, args.timeout)
            if not mobile.get("ok"):
                result = {"body": mobile.get("reply") or "", "sources": [], "url": ""}
            else:
                # Reconnect after the potentially long mobile generation so an idle
                # DevTools websocket cannot break the web extraction phase.
                web = AfuWebCollector(args.chrome_port)
                result = web.collect_latest(previous, args.timeout)
            answer = str(result.get("body") or mobile.get("reply") or "")
            skip = answer_quality_reason(prompt, answer)
            row = {"status": "skipped" if skip else "success", "skip_reason": skip,
                   "round": index + 1, "serial": args.serial, "prompt": prompt,
                   "question": canonical_recommendation_question(prompt), "reply": answer,
                   "web_body": answer, "sources": [] if skip else result.get("sources", []),
                   "page_url": result.get("url"), "mobile": mobile,
                   "started_at": started, "finished_at": now()}
            append(Path(args.results), row)
            log.info("第 %d 轮完成：%s，正文 %d 字，信源 %d 条", index + 1, prompt, len(answer), len(row["sources"]))
        except Exception as exc:
            append(Path(args.results), {"status": "error", "round": index + 1, "prompt": prompt,
                                        "question": canonical_recommendation_question(prompt),
                                        "error": str(exc), "started_at": started, "finished_at": now()})
            log.exception("第 %d 轮失败：%s", index + 1, exc)
        index += 1
