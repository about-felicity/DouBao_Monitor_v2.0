...
$dashboardRoot = Join-Path $projectRoot "dashboard\frontend"
$runtimeRoot = Join-Path $projectRoot "runtime\unified_control"
New-Item -ItemType Directory -Force -Path $runtimeRoot | Out-Null

# The central services have moved to the LAN server. Keep this familiar entry
# point useful without accidentally starting a second database/API stack on the
# collector workstation.
$serverConfigPath = Join-Path $projectRoot "local_server_config.txt"
if (Test-Path -LiteralPath $serverConfigPath) {
    $serverConfig = Get-Content -LiteralPath $serverConfigPath -Encoding UTF8 -Raw
    $serverMatch = [regex]::Match($serverConfig, '@(?<host>(?:\d{1,3}\.){3}\d{1,3})(?::\d+)?')
    if ($serverMatch.Success) {
        $centralHost = $serverMatch.Groups['host'].Value
        $centralDashboard = "http://${centralHost}:3000"
        $centralStatus = "http://${centralHost}:8765/api/control/status"
        try {
            $response = Invoke-WebRequest -Uri $centralStatus -UseBasicParsing -TimeoutSec 10
            if ($response.StatusCode -ne 200) { throw "HTTP $($response.StatusCode)" }
        }
        catch {
            throw "中心服务器不可用：$centralStatus；不会回退启动本机中心服务。$($_.Exception.Message)"
        }
        if (-not $NoOpen) { Start-Process $centralDashboard }
        Write-Host "中心数据库、接收器、分析服务和面板已迁移：$centralDashboard"
        return
    }
}
...
[Environment]::SetEnvironmentVariable("MONITOR_SOURCE_INTERSECTION_WORKERS", "1", "Process")
[Environment]::SetEnvironmentVariable("MONITOR_SOURCE_INTERSECTION_BACKGROUND_LIMIT", "1", "Process")
if (-not [Environment]::GetEnvironmentVariable("MONITOR_DATABASE_STATEMENT_TIMEOUT_MS", "Process")) {
    [Environment]::SetEnvironmentVariable("MONITOR_DATABASE_STATEMENT_TIMEOUT_MS", "120000", "Process")
}
if (-not [Environment]::GetEnvironmentVariable("MONITOR_DATABASE_MAX_PARALLEL_WORKERS", "Process")) {
    [Environment]::SetEnvironmentVariable("MONITOR_DATABASE_MAX_PARALLEL_WORKERS", "1", "Process")
}
[Environment]::SetEnvironmentVariable("MONITOR_DATABASE_RETENTION_DAYS", "60", "Process")
...
if (-not (Test-ListeningPort 8791)) {
    ...
    Start-Process -FilePath "python" -ArgumentList @("-u", (Join-Path $projectRoot "transport\receiver.py")) `
...
if (-not (Test-ListeningPort 8765)) {
    ...
    Start-Process -FilePath "python" -ArgumentList @("-u", (Join-Path $projectRoot "dashboard\backend\server.py")) `
...
Start-AnalysisWorker "services\analysis\product_worker.py" "remote_product_ai_worker"
