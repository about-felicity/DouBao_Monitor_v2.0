...
from deployment.build_remote_model_packages import MODELS, build_package, clean_generated_packages, copy_source_tree
...
class RemoteModelPackageTests(unittest.TestCase):
    ...
    def test_package_embeds_pairing_and_only_one_plugin(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                [path.parent.name for path in (folder / "collectors" / "plugins").glob("*/plugin.py")],
                ["deepseek"],
            )
