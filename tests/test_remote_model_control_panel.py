...
from dashboard.backend.control_panel import (
    RemoteModelPanel,
    account_gate_open,
...
from collectors.wenxin import controller as wenxin_controller
...
class RemoteModelPanelTest(unittest.TestCase):
    ...
    def test_wenxin_baidu_check_does_not_compete_with_running_collector(self):
        ...
        thread = mock.Mock()
        with mock.patch("dashboard.backend.control_panel.threading.Thread", return_value=thread), \
                mock.patch("dashboard.backend.control_panel.remote_collector_running", return_value=True):
            panel.check_account()
        self.assertFalse(panel.account_check_running)
        thread.start.assert_not_called()
        panel.status_var.set.assert_called_once_with("运行中（已有采集任务）")
        panel.start_button.configure.assert_called_once_with(state="disabled")
    ...
    def test_worker_uses_console_python_when_panel_runs_under_pythonw(self) -> None:
        with mock.patch("dashboard.backend.control_panel.sys.executable", r"C:\Python\pythonw.exe"), \
                mock.patch("dashboard.backend.control_panel.Path.is_file", return_value=True):
    ...
    def test_panel_stop_terminates_its_worker_process_tree(self) -> None:
        ...
        panel.append_log = mock.Mock()
        with mock.patch("dashboard.backend.control_panel.os.name", "nt"), \
                mock.patch("dashboard.backend.control_panel.subprocess.run") as run:
            panel.stop()
