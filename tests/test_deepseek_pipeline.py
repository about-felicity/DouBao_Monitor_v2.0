...
DEEPSEEK_DIR = Path(__file__).resolve().parent.parent / "collectors" / "deepseek" / "legacy_app"
if str(DEEPSEEK_DIR) not in sys.path:
    sys.path.insert(0, str(DEEPSEEK_DIR))

import controller
import deepseek_loop
from collectors.plugins.deepseek.plugin import Plugin as DeepSeekPlugin
