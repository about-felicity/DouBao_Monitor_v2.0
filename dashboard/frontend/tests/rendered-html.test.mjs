...
test("统一面板由后端模型目录驱动并提供完整分析工作视图", async () => {
  ...
  assert.doesNotMatch(source, /item\.id !== "deepseek"/);
  assert.match(source, /const visibleModelCatalog = analytics\.model_catalog;/);
  assert.match(source, /filter\(\(item\) => !model \|\| item\.id === model\)/);
...
test("信源分析区显示每日 Top 25、关键词和三模型自有链接交集", async () => {
  const source = await readFile(new URL("Dashboard.tsx", app), "utf8");
  assert.match(source, /信源统计月份/);
  assert.match(source, /所选自然月全部信源/);
  assert.match(source, /不混入其他月份/);
...
test("总览提供每日自有产品跨模型上榜看板并区分缺数与负样本", async () => {
  ...
  assert.match(source, /\$\{mentionRate\.toFixed\(1\)\}%/);
  assert.match(source, /owned-product-rate/);
  assert.match(source, /owned_product_daily/);
  assert.match(source, /owned_product_summary/);
  assert.match(source, /已跑天数汇总/);
  assert.match(source, /日均提及率月份/);
  assert.match(source, /summary_month/);
  assert.match(source, /仅统计所选自然月/);
  assert.match(source, /平均每天提及率/);
  assert.match(source, /不受提问次数影响/);
  assert.match(source, /该模型没数据的日期不计入/);
  assert.match(source, /row\.average_daily_mention_rate\.toFixed\(1\)/);
  assert.match(source, /models\.map\(\(item\) => <th key=\{item\.id\}>\{item\.id === "wenxin" \? "文心日均（双卡片）"/);
  assert.match(source, /wenxin-surface-stack summary/);
  assert.match(source, /最近有效/);
  assert.match(source, /无有效卡片/);
  assert.match(source, /const display = stateText\(row\.models\[item\.id\]\)/);
});

test("DeepSeek 回答审计在展示前剔除搜索界面壳文本", async () => {
  const source = await readFile(new URL("Dashboard.tsx", app), "utf8");
  assert.match(source, /function cleanAuditAnswer/);
  assert.match(source, /web\\s\*pages/);
  assert.match(source, /cleanAuditAnswer\(run\.answer, run\.model\.id\)/);
});

test("总览按每日目标和个人权重展示人员绩效", async () => {
  const dashboard = await readFile(new URL("Dashboard.tsx", app), "utf8");
  const performance = await readFile(new URL("PerformanceBoard.tsx", app), "utf8");
  assert.match(dashboard, /PerformanceBoard/);
  assert.match(dashboard, /employee_performance/);
  assert.match(performance, /每日人员绩效/);
  assert.match(performance, /综合绩效/);
  assert.match(performance, /有效权重/);
  assert.match(performance, /单项最高计200%/);
  assert.match(performance, /收起绩效面板/);
  assert.match(performance, /展开绩效面板/);
  assert.match(performance, /文心双卡各50%/);
  assert.match(performance, /最终得分怎么算/);
  assert.match(performance, /展开完整计算明细/);
  assert.match(performance, /实际概率/);
  assert.match(performance, /目标概率/);
});
