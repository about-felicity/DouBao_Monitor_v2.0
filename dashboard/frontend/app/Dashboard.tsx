...
import { PerformanceBoard, type EmployeePerformance } from "./PerformanceBoard";
...
type OwnedProductSurfaceStatus = OwnedProductRateStatus & {
  surface_label?: string;
  attempted_runs?: number;
  raw_eligible_runs?: number;
  duplicate_runs_merged?: number;
  unavailable_runs?: number;
  target_runs?: number;
  availability_note?: string;
};
type OwnedProductModelStatus = OwnedProductRateStatus & {
  surfaces?: Record<string, OwnedProductSurfaceStatus>;
};
...
type OwnedProductSummaryStatus = {
  observed_days: number;
  average_daily_mention_rate: number;
  total_recommendation_runs: number;
  total_eligible_runs: number;
  pooled_mention_rate: number;
  previous_average_daily_mention_rate?: number | null;
  day_over_day_change?: number | null;
  surfaces?: Record<string, OwnedProductSummarySurfaceStatus>;
};
type OwnedProductSummarySurfaceStatus = Omit<OwnedProductSummaryStatus, "surfaces"> & {
  surface_label?: string;
  first_date?: string;
  last_date?: string;
...
type OwnedProductSummaryRow = OwnedProductSummaryStatus & {
  question: string;
  product: string;
  brand: string;
  first_date: string;
  last_date: string;
  observed_model_days: number;
  models: Record<string, OwnedProductSummaryStatus>;
};
type Model = {
  id: string;
  name: string;
...
type Analytics = {
  generated_at: string;
  filters?: { model?: string; question?: string; date?: string; view?: string; summary_month?: string };
  detail_scope?: {
    kind: "latest_day" | "history_window";
    dates: string[];
    date_from: string;
    date_to: string;
  };
  ...
  owned_product_summary?: OwnedProductSummaryRow[];
  owned_product_summary_month?: string;
  owned_product_summary_months?: string[];
...
function cleanAuditAnswer(value: string, modelId: string) {
  if (modelId !== "deepseek") return value;
  const shellLine = /^(?:found\s+)?\d{1,3}\s*(?:(?:个)?网页|篇来源|web\s*pages?|sources?)$/i;
  const lines = String(value || "").replace(/\r\n?/g, "\n").trim().split("\n");
  while (lines.length && !lines[0].trim()) lines.shift();
  while (lines.length && !lines.at(-1)?.trim()) lines.pop();
  while (lines.length && shellLine.test(lines[0].trim())) lines.shift();
  while (lines.length && shellLine.test(lines.at(-1)?.trim() || "")) lines.pop();
  ...
  for (const line of lines) {
    ...
    if (
      ...
      cleaned[cleaned.length - 1] = previous.slice(0, -1).trimEnd();
      continue;
    }
    if (cleaned.length && /^[，。！？；：,.!?;:]+$/.test(current)) {
      while (cleaned.length && !cleaned.at(-1)?.trim()) cleaned.pop();
      if (cleaned.length) cleaned[cleaned.length - 1] = `${cleaned.at(-1)?.trimEnd()}${current}`;
      continue;
    }
...
function DeltaIndicator({ value, unavailable = "昨日无数据" }: { value?: number | null; unavailable?: string }) {
  if (value === undefined || value === null) return <small className="rate-delta unavailable">{unavailable}</small>;
  const normalized = Math.abs(value) < 0.005 ? 0 : value;
  const className = normalized > 0 ? "increase" : normalized < 0 ? "decrease" : "unchanged";
  const prefix = normalized > 0 ? "↑ +" : normalized < 0 ? "↓ " : "→ ";
  return <small className={`rate-delta ${className}`}>较昨日 {prefix}{normalized.toFixed(1)} 个百分点</small>;
}
...
function DailyOwnedProductBoard({
  ...
  summary: OwnedProductSummaryRow[];
  models: Model[];
  summaryMonth: string;
  ...
  const stateText = (status?: OwnedProductModelStatus) => {
    if (!status || status.state === "not_collected") return { label: "未采集", detail: "没有对应问题", rate: null, className: "not-collected" };
    const mentionRate = Number.isFinite(status.recommendation_rate)
      ? status.recommendation_rate
      : status.eligible_runs
        ? status.recommendation_runs * 100 / status.eligible_runs
        : 0;
    const mentionRateText = `${mentionRate.toFixed(1)}%`;
    ...
    return { label: "未上榜", detail: `0/${status.eligible_runs} 轮推荐`, rate: mentionRateText, className: "not-listed" };
  };
  const surfaceOrder: [string, string][] = [["wenxin_card", "文心卡片"], ["baidu_card", "百度卡片"]];
  const dailySurfaceCells = (status?: OwnedProductModelStatus) => (
    <div className="wenxin-surface-stack daily">
      {surfaceOrder.map(([surfaceId, fallbackLabel]) => {
        const surface = status?.surfaces?.[surfaceId];
        const label = surface?.surface_label || fallbackLabel;
        if (!surface || !surface.eligible_runs) {
          const attempted = surface?.attempted_runs || 0;
          const target = surface?.target_runs || 0;
          const detail = surface?.availability_note || (surfaceId === "baidu_card" ? "今日尚未检测百度卡片" : "暂无有效数据");
          const baiduPending = surfaceId === "baidu_card" && target > 0 && attempted < target;
          const baiduComplete = surfaceId === "baidu_card" && target > 0 && attempted >= target;
          const emptyLabel = baiduPending ? "检测中" : baiduComplete ? "无卡片" : "—";
          const emptyDetail = baiduPending
            ? `已探测 ${attempted}/${target} 轮`
            : baiduComplete
              ? `${target} 轮均无有效卡片`
              : detail;
          return <span className="owned-product-surface empty" key={surfaceId}><em>{label}</em><b>{emptyLabel}</b><small>{emptyDetail}</small></span>;
        }
        const display = stateText(surface);
        const raw = surface.raw_eligible_runs || surface.eligible_runs;
        const merged = surface.duplicate_runs_merged || 0;
        return <span className={`owned-product-surface ${display.className}`} key={surfaceId}><em>{label}</em><span className="owned-product-state-head"><b>{display.label}</b><strong className="owned-product-rate">{display.rate}</strong></span><small>{display.detail}</small>{surfaceId === "baidu_card" && <small>{raw}轮采集 → {surface.eligible_runs}个有效样本{merged ? ` · 合并${merged}轮重复` : ""}</small>}<DeltaIndicator value={surface.day_over_day_change} /></span>;
      })}
    </div>
  );
  const summarySurfaceCells = (status?: OwnedProductSummaryStatus) => (
    <div className="wenxin-surface-stack summary">
      {surfaceOrder.map(([surfaceId, fallbackLabel]) => {
        const surface = status?.surfaces?.[surfaceId];
        const label = surface?.surface_label || fallbackLabel;
        if (!surface?.observed_days) {
          return <span className="owned-product-model-average owned-product-surface empty" key={surfaceId}><em>{label}</em><b>—</b><small>{surface?.availability_note || "暂无有效数据"}</small></span>;
        }
        const merged = surface.duplicate_runs_merged || 0;
        const firstDate = surface.first_date || "";
        const lastDate = surface.last_date || "";
        const lastAttemptDate = surface.last_attempt_date || lastDate;
        const observedDateNote = lastDate && lastAttemptDate > lastDate
          ? `最近有效 ${lastDate.slice(5)} · ${lastAttemptDate.slice(5)}无有效卡片`
          : firstDate && lastDate && firstDate !== lastDate
            ? `有效日期 ${firstDate.slice(5)} 至 ${lastDate.slice(5)}`
            : lastDate
              ? `有效日期 ${lastDate.slice(5)}`
              : "";
        return <span className="owned-product-model-average owned-product-surface" key={surfaceId}><em>{label}</em><b>{surface.average_daily_mention_rate.toFixed(1)}%</b><small>{surface.observed_days}天 · {surface.total_recommendation_runs}/{surface.total_eligible_runs} 样本</small>{observedDateNote ? <small className="surface-date-note">{observedDateNote}</small> : null}{surfaceId === "baidu_card" && merged ? <small>原始{surface.raw_eligible_runs || surface.total_eligible_runs}轮 · 合并{merged}轮重复</small> : null}<DeltaIndicator value={surface.day_over_day_change} unavailable="暂无昨日基准" /></span>;
      })}
    </div>
  );
  return (
    <section className="panel owned-product-board">
      ...
      {summary.length ? (
        <div className="owned-product-summary">
          <div className="owned-product-summary-heading">
            <div><b>{summaryMonth || "当月"} 已跑天数汇总</b><small>仅统计所选自然月；每个“产品 × 模型 × 日期”的提及率等权平均，不受提问次数影响，该模型没数据的日期不计入</small></div>
            <div className="owned-product-summary-controls">
              <label>月份<select aria-label="日均提及率月份" value={summaryMonth} onChange={(event) => onSummaryMonthChange(event.target.value)}>{summaryMonths.map((month) => <option key={month} value={month}>{month}</option>)}</select></label>
              <span>{summary.length} 个产品</span>
            </div>
          </div>
          <div className="owned-product-table owned-product-summary-table">
            <table>
              <thead><tr><th>我的产品</th><th>对应问题</th><th>有数据天数</th><th>平均每天提及率</th><th>累计推荐 / 有效回答</th>{models.map((item) => <th key={item.id}>{item.id === "wenxin" ? "文心日均（双卡片）" : `${item.name} 日均`}</th>)}</tr></thead>
              <tbody>{summary.map((row) => (
                <tr key={`${row.question}-${row.product}`}>
                  <td><b>{row.product}</b><small>{row.brand}</small></td>
                  <td>{row.question}</td>
                  <td><b>{row.observed_days} 天</b><small>{row.first_date}{row.first_date && row.last_date && row.first_date !== row.last_date ? ` 至 ${row.last_date}` : ""}</small></td>
                  <td><strong className="owned-product-summary-rate">{row.average_daily_mention_rate.toFixed(1)}%</strong><small>{row.observed_model_days} 个有效模型日</small><DeltaIndicator value={row.day_over_day_change} unavailable="暂无昨日基准" /></td>
                  <td><b>{row.total_recommendation_runs} / {row.total_eligible_runs}</b><small>跨日累计，仅作样本量参考</small></td>
                  {models.map((item) => {
                    const status = row.models[item.id];
                    return <td key={item.id}>{item.id === "wenxin" && status?.surfaces ? summarySurfaceCells(status) : status?.observed_days ? <span className="owned-product-model-average"><b>{status.average_daily_mention_rate.toFixed(1)}%</b><small>{status.observed_days} 个有数据日 · {status.total_recommendation_runs}/{status.total_eligible_runs} 轮</small><DeltaIndicator value={status.day_over_day_change} unavailable="暂无昨日基准" /></span> : <span className="owned-product-model-average empty"><b>—</b><small>暂无有效数据</small></span>}</td>;
                  })}
                </tr>
              ))}</tbody>
            </table>
      ...
      {groups.length ? groups.map(([day, dayRows]) => (
        <div className="owned-product-day" key={day}>
          ...
          <div className="owned-product-table owned-product-daily-table">
            <table>
              <thead><tr><th>我的产品</th><th>对应问题</th>{models.map((item) => <th className={item.id === "wenxin" ? "wenxin-daily-column" : undefined} key={item.id}>{item.id === "wenxin" ? "文心（双卡片）" : item.name}</th>)}</tr></thead>
              <tbody>{dayRows.map((row) => (
                <tr key={`${row.date}-${row.question}-${row.product}`}>
                  ...
                  {models.map((item) => {
                    ...
                    return <td className={item.id === "wenxin" ? "wenxin-daily-column" : undefined} key={item.id}>{item.id === "wenxin" && row.models[item.id]?.surfaces ? dailySurfaceCells(row.models[item.id]) : <span className={`owned-product-state ${display.className}`}><span className="owned-product-state-head"><b>{display.label}</b>{display.rate !== null ? <strong className="owned-product-rate" aria-label={`提及率 ${display.rate}`}>{display.rate}</strong> : null}</span><small>{display.detail}</small>{day === groups[0]?.[0] && display.rate !== null ? <DeltaIndicator value={row.models[item.id]?.day_over_day_change} /> : null}</span>}</td>;
                  })}
                </tr>
              ))}</tbody>
            </table>
...
function BrandProductTrendExplorer({ model, question, focusDate, ownedBrands, loadHistory }: { model: Model; question: string; focusDate: string; ownedBrands: { name: string; aliases: string[] }[]; loadHistory: boolean }) {
  ...
  return <section className="brand-trend-explorer">
    <div className="trend-explorer-head"><div><span>INTERACTIVE TREND</span><h3>品牌提及与信源</h3><small>{historyLoading ? "当天数据已显示；最近 14 个自然日趋势正在后台补齐…" : allDates.length > 1 ? "选择任意品牌；折线展示当前问题最近 14 个自然日，点击日期可查看该品牌信源" : loadHistory ? "当天数据已显示；需要时再加载最近 14 个自然日趋势，不阻塞模型切换" : "综合比较先展示所选日期；选择单个模型可查看最近 14 个自然日趋势"}</small></div>{loadHistory && !historicalModel && <button type="button" className="trend-more" disabled={historyLoading} onClick={() => { setHistoryLoading(true); setHistoryRequested(true); }}>{historyLoading ? "趋势加载中…" : "加载14日趋势"}</button>}</div>
    <div className="trend-selectors brand-only"><label>选择品牌<select value={activeBrand} onChange={(event) => setBrand(event.target.value)}>{brandOptions.map((name) => <option key={name}>{name}</option>)}</select></label></div>
    ...
    <div className="competitor-links"><div className="competitor-links-head"><div><b>{activeBrand || "品牌"}信源链接</b><small>{activeDate || "请选择折线日期"} · {selectedSourceEligible ? `${selectedSourceMentions}/${selectedSourceEligible} 个已分析唯一信源提及（${selectedSourceRate.toFixed(1)}%） · 共观测 ${selectedSourceObserved} 个${selectedSourcePending ? `，${selectedSourcePending} 个待正文分析` : ""}${selectedSourceFailed ? `，${selectedSourceFailed} 个抓取失败` : ""}${selectedLinks ? ` · ${selectedLinks.links.length} 条可查看链接` : ""}` : selectedSourceObserved ? `共观测 ${selectedSourceObserved} 个${selectedSourcePending ? `，${selectedSourcePending} 个待正文分析` : ""}${selectedSourceFailed ? `，${selectedSourceFailed} 个抓取失败` : ""}` : "该日无信源"}</small></div>{linkLoading && <em>读取中…</em>}</div>
      {!!linkDays.length && <div className="link-date-tabs" aria-label="选择品牌信源日期">{linkDays.map((day) => <button key={day.date} type="button" className={activeDate === day.date ? "active" : ""} onClick={() => setSelectedDate(day.date)}><span>{day.date.slice(5)}</span><b>{day.mentions}</b></button>)}</div>}
...
export function Dashboard() {
  ...
  const [summaryMonth, setSummaryMonth] = useState(() => new Intl.DateTimeFormat("en-CA", {
  ...
  const sourceEntryDate = useRef("");

  const selectView = (nextView: View) => {
    if (nextView === "sources" && view !== "sources") {
      sourceEntryDate.current = date;
      setDate("");
    } else if (view === "sources" && nextView !== "sources" && !date && sourceEntryDate.current) {
      setDate(sourceEntryDate.current);
    }
    setView(nextView);
  };
  ...
  const analytics = useMemo<Analytics>(() => {
    ...
    if (
      ...
      (filters?.date || "") === date &&
      (filters?.view || "") === view &&
      (!["overview", "sources"].includes(view) || (filters?.summary_month || "") === summaryMonth)
    ) return loadedAnalytics;
    return {
      ...emptyAnalytics,
      filters: { model, question, date, view, summary_month: summaryMonth },
  ...
  }, [loadedAnalytics, model, question, date, view, summaryMonth]);
  ...
  const loadAnalytics = useCallback(async (force = false, silent = false) => {
    ...
    const cacheKey = JSON.stringify([model, question, date, view, summaryMonth]);
    ...
    try {
      ...
      if (["overview", "sources"].includes(view) && summaryMonth) params.set("summary_month", summaryMonth);
  ...
  }, [model, question, date, view, summaryMonth, filtersReady]);
  ...
  // Keep every configured model visible even with zero rows so "未采集" is
  // represented explicitly and cross-model comparisons keep stable columns.
  const visibleModelCatalog = analytics.model_catalog;
  const selectedModels = analytics.models.filter((item) => !model || item.id === model);
    ...
    <main className="app-shell">
        ...
        <div className="brand">
          ...
          <span>
            ...
            <span className="brand-meta">
              ...
              <a className="enterprise-entry" href="/enterprise">企业概率监控</a>
            </span>
          </span>
        </div>
        <div className="scope">
      ...
      <section className="workspace">
              ...
              <b>
                {model
                  ? analytics.model_catalog.find((item) => item.id === model)
                      ?.name
                  : "全部模型"}{" "}
                · {question || "全部问题"} · {date || (view === "sources" ? `${summaryMonth}整月` : "全部日期")}
              </b>
            </div>
          </div>
        )}
        ...
        <div className="content">
          ...
          {view === "overview" && (
            <>
              {!model && !question && <PerformanceBoard payload={analytics.employee_performance} />}
              <DailyOwnedProductBoard
                rows={analytics.owned_product_daily || []}
                summary={analytics.owned_product_summary || []}
                models={selectedModels}
                summaryMonth={analytics.owned_product_summary_month || summaryMonth}
                summaryMonths={analytics.owned_product_summary_months?.length ? analytics.owned_product_summary_months : [summaryMonth]}
          ...
          {view === "sources" && (
            <>
              ...
              {!date && analytics.detail_scope?.kind === "month_window" && (
                <section className="analysis-note panel source-scope-note">
                  <b>{analytics.detail_scope.month || summaryMonth} 产品信源已加载</b>
                  <span>
                    当前已按“{question} × {selectedModels[0]?.name || "所选模型"}”读取所选自然月全部信源；Top 25 的次数、排序及上方 KPI 均严格使用同一月份，不混入其他月份。
                  </span>
                </section>
              )}
                ...
                <section className="panel source-section" key={item.id}>
                  <SectionTitle
                    ...
                    note={date || (analytics.detail_scope?.date_from && analytics.detail_scope?.date_to
                      ? analytics.detail_scope.kind === "month_window"
                        ? `${analytics.detail_scope.date_from} 至 ${analytics.detail_scope.date_to}`
                        : `${analytics.detail_scope.date_to} · 最新有效日明细`
                      : "最新有效日明细")}
                  />
                  {!date && analytics.detail_scope?.kind === "month_window" && (
                    <div className="daily-source-group source-history-summary">
                      <h3>{analytics.detail_scope.month || summaryMonth} · 整月汇总</h3>
                      <div className="source-pair">
                        <SourceTop title="高频文章 Top 25（本月汇总）" items={item.top_articles} tone="article" />
                        <SourceTop title="高频视频 Top 25（本月汇总）" items={item.top_videos} tone="video" />
                      </div>
                    </div>
                  )}
                    ...
                    <article key={`${run.model.id}-${run.run_id}`}>
                      ...
                      <details>
                        ...
                        <div className="answer-text">
                          {cleanAuditAnswer(run.answer, run.model.id) || "未保存回答正文"}
                        </div>
                        <ul>
                    ...
                    >
                      ...
                      {item.ingest_only && (
                        <div className="account-result ingest-only">
                          <b>接收端已启用：POST /api/v1/models/{item.id}/results</b>
