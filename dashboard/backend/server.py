...
from legacy.doubao.core import doubao_question_aliases as qa
from legacy.doubao.core import doubao_brand_settings as brand_settings
...
from monitor_core.performance import (
...
CONTENT_WORKER_PATH = os.path.join(BASE_DIR, "services", "analysis", "source_content_worker.py")
QUESTION_ALIASES_PATH = os.path.join(BASE_DIR, "legacy", "doubao", "core", "doubao_question_aliases.py")
BRAND_SETTINGS_PATH = str(brand_settings.SETTINGS_PATH)
DEBUG_LOG_PATH = os.path.join(BASE_DIR, "doubao_run_debug.log")
RAG_ML_LAB_PATH = os.path.join(BASE_DIR, "dashboard", "static", "doubao_rag_ml_lab.html")
...
OWNED_PRODUCT_COMPARISON_SCHEMA_VERSION = 2

ENTERPRISE_MONITOR_QUESTION = "成都做GEO比较好的公司推荐一下"
ENTERPRISE_MONITOR_QUESTIONS = (
    ENTERPRISE_MONITOR_QUESTION,
    ENTERPRISE_MONITOR_QUESTION + "？",
    ENTERPRISE_MONITOR_QUESTION + "?",
)
ENTERPRISE_OWNED_COMPANY = "成都巨量求索科技有限责任公司"
ENTERPRISE_COMPANY_ALIASES = (
    ENTERPRISE_OWNED_COMPANY,
    "成都巨量求索科技",
    "成都巨量求索",
    "巨量求索科技",
    "巨量求索",
...
_ANALYTICS_CACHE_SCHEMA_VERSION = 46
...
_ANALYTICS_CACHE_MAX = max(
    4, min(32, int(os.environ.get("MONITOR_ANALYTICS_MEMORY_CACHE_MAX", "12") or 12))
)
_RETAIN_ANALYTICS_SNAPSHOT = (
    os.environ.get("MONITOR_RETAIN_GLOBAL_SNAPSHOT", "0").strip() != "0"
)
_OWNED_BOARD_CACHE = {}
...
_SOURCE_INTERSECTION_CACHE_MAX = max(
    2,
    min(
        16,
        int(os.environ.get("MONITOR_SOURCE_INTERSECTION_MEMORY_CACHE_MAX", "8") or 8),
    ),
)
...
_SOURCE_INTERSECTION_BUILD_SEMAPHORE = threading.Semaphore(
    max(1, min(2, int(os.environ.get("MONITOR_SOURCE_INTERSECTION_WORKERS", "1") or 1)))
)
_SOURCE_INTERSECTION_BACKGROUND_LIMIT = max(
    1, min(2, int(os.environ.get("MONITOR_SOURCE_INTERSECTION_BACKGROUND_LIMIT", "1") or 1))
)
...
def _analytics_content_token():
    version = [_ANALYTICS_CACHE_SCHEMA_VERSION, BRAND_MATCH_SCHEMA_VERSION,
               OWN_PRODUCT_SCHEMA_VERSION, OWNED_PRODUCT_COMPARISON_SCHEMA_VERSION,
               PERFORMANCE_SCHEMA_VERSION]
...
def _analytics_snapshot(version):
    ...
    with _ANALYTICS_SNAPSHOT_LOCK:
        ...
        if monitor_database.enabled():
            try:
                database_runs = monitor_database.load_runs_by_model(
                    exclude_questions=ENTERPRISE_MONITOR_QUESTIONS)
                if database_runs:
                    runs_by_model = {
        ...
        if runs_by_model is None:
            runs_by_model = {
                model_id: [row for row in plugin.analytics_runs()
                           if not _is_enterprise_question(row.get("question"))]
...
def _remember_source_intersection_locked(key, value):
    """Bound intersection payloads retained by the long-running API process."""
    _SOURCE_INTERSECTION_CACHE.pop(key, None)
    _SOURCE_INTERSECTION_CACHE[key] = value
    while len(_SOURCE_INTERSECTION_CACHE) > _SOURCE_INTERSECTION_CACHE_MAX:
        oldest_key = next(iter(_SOURCE_INTERSECTION_CACHE))
        _SOURCE_INTERSECTION_CACHE.pop(oldest_key, None)
        _SOURCE_INTERSECTION_LAST_VALIDATED.pop(oldest_key, None)
        _SOURCE_INTERSECTION_LOCKS.pop(oldest_key, None)
...
def _is_enterprise_question(value):
    key = re.sub(r"[\s？?。！!]+", "", str(value or "")).casefold()
    target = re.sub(r"[\s？?。！!]+", "", ENTERPRISE_MONITOR_QUESTION).casefold()
    return key == target


def _enterprise_company_mentioned(answer):
    key = re.sub(r"[^0-9a-z\u4e00-\u9fff]+", "", str(answer or "")).casefold()
    return any(re.sub(r"[^0-9a-z\u4e00-\u9fff]+", "", alias).casefold() in key
               for alias in ENTERPRISE_COMPANY_ALIASES)


def _enterprise_analytics(params):
    model_filter = (params.get("model") or [""])[0]
    day_filter = (params.get("date") or [""])[0]
    if model_filter and model_filter not in MODEL_PLUGINS:
        raise ValueError("未知模型")
    combined = defaultdict(list)
    all_dates = set()
    if monitor_database.enabled():
        # Incoming collectors use the configured question verbatim. Keep this
        # indexed lookup singular; repeated variant scans also repeat all child
        # source/brand/product joins on a mature database.
        loaded = monitor_database.load_runs_by_model(
            question=ENTERPRISE_MONITOR_QUESTION, model=model_filter, day=day_filter,
            include_analysis=False)
        for model_id, rows in loaded.items():
            combined[model_id].extend(rows)
        all_dates.update(monitor_database.analytics_dates(
            model=model_filter, question=ENTERPRISE_MONITOR_QUESTION))
    else:
        for model_id, plugin in MODEL_PLUGINS.items():
            if model_filter and model_id != model_filter:
                continue
            rows = [row for row in plugin.analytics_runs()
                    if _is_enterprise_question(row.get("question"))]
            all_dates.update(filter(None, (analytics_beijing_day(row) for row in rows)))
            combined[model_id] = [row for row in rows
                                  if not day_filter or analytics_beijing_day(row) == day_filter]

    model_rows, recent = [], []
    daily = defaultdict(lambda: {"valid_answers": 0, "owned_mentions": 0, "models": set()})
    total_received = total_valid = total_mentions = total_failed = 0
    for model_id, metadata in MODEL_REGISTRY.items():
        if model_filter and model_id != model_filter:
            continue
        rows, valid, failed = combined.get(model_id, []), [], 0
        for row in rows:
            answer = str(row.get("answer") or "").strip()
            ok = (str(row.get("status") or "success").casefold() == "success"
                  and bool(answer) and row.get("body_capture_complete") is not False)
            failed += int(not ok)
            if ok:
                valid.append(row)
            mentioned = bool(ok and _enterprise_company_mentioned(answer))
            day = analytics_beijing_day(row)
            if ok and day:
                daily[day]["valid_answers"] += 1
                daily[day]["owned_mentions"] += int(mentioned)
                daily[day]["models"].add(model_id)
            recent.append({"model_id": model_id, "model_name": metadata.get("name") or model_id,
                           "run_id": str(row.get("run_id") or ""), "date": day,
                           "finished_at": str(row.get("finished_at") or row.get("captured_at") or ""),
                           "status": "success" if ok else "failed", "mentioned": mentioned,
                           "answer": answer,
                           "sources": [{"title": str(s.get("title") or s.get("name") or "信源"),
                                        "url": str(s.get("url") or s.get("link") or "")}
                                       for s in (row.get("sources") or []) if isinstance(s, dict)]})
        mentions = sum(_enterprise_company_mentioned(row.get("answer")) for row in valid)
        total_received += len(rows); total_valid += len(valid)
        total_mentions += mentions; total_failed += failed
        model_rows.append({"id": model_id, "name": metadata.get("name") or model_id,
                           "tone": metadata.get("tone") or "green", "received": len(rows),
                           "valid_answers": len(valid), "failed": failed,
                           "owned_mentions": mentions,
                           "mention_rate": round(mentions * 100 / len(valid), 1) if valid else 0.0})
    recent.sort(key=lambda row: (row["finished_at"], row["run_id"]), reverse=True)
    daily_rows = [{"date": day, "valid_answers": item["valid_answers"],
                   "owned_mentions": item["owned_mentions"], "model_count": len(item["models"]),
                   "mention_rate": round(item["owned_mentions"] * 100 / item["valid_answers"], 1)
                   if item["valid_answers"] else 0.0}
                  for day, item in sorted(daily.items(), reverse=True)]
    return {"ok": True, "generated_at": beijing_now(),
            "question": ENTERPRISE_MONITOR_QUESTION, "owned_company": ENTERPRISE_OWNED_COMPANY,
            "dates": sorted(all_dates, reverse=True), "models": model_rows,
            "daily": daily_rows, "recent_runs": recent[:100],
            "totals": {"received": total_received, "valid_answers": total_valid,
                       "failed": total_failed, "owned_mentions": total_mentions,
                       "mention_rate": round(total_mentions * 100 / total_valid, 1) if total_valid else 0.0,
                       "model_count": sum(row["valid_answers"] > 0 for row in model_rows)}}
...
def _build_unified_analytics(cache_key, version):
    ...
    if _uses_scoped_database_load(cache_key):
        ...
        if not cache_key[1]:
            ...
            detailed_product_model_sources = bool(
                cache_key[3] == "sources" and cache_key[0] and cache_key[2]
            )
            detail_limits = {
                "overview": 7, "brands": 2, "runs": 1,
                "compare": 1, "sources": 60 if detailed_product_model_sources else 1,
            }
            all_detail_days = list(filter_options.get("dates") or [])
            requested_month = str(cache_key[4] if len(cache_key) > 4 else "").strip()
            if detailed_product_model_sources:
                if not re.fullmatch(r"\d{4}-\d{2}", requested_month):
                    requested_month = (all_detail_days[0][:7] if all_detail_days else datetime.now(CST).strftime("%Y-%m"))
                detail_days = [
                    day for day in all_detail_days
                    if str(day).startswith(f"{requested_month}-")
                ]
                month_start = datetime.strptime(f"{requested_month}-01", "%Y-%m-%d").date()
                next_month = (
                ...
                load_scope = {
                    "day_from": month_start.isoformat(),
                    "day_to": (next_month - timedelta(days=1)).isoformat(),
                }
            else:
                detail_days = all_detail_days[:detail_limits.get(cache_key[3], 1)]
            if detail_days and not detailed_product_model_sources:
                load_scope = {
                    "day_from": min(detail_days),
                    "day_to": max(detail_days),
                }
            # A selected product/model source drill-down is a calendar-month
            # view. Its KPI totals must come from the same loaded month as its
            # Top 25 rows, not from the all-history summary.
            if not detailed_product_model_sources:
            ...
            if cache_key[3] == "sources":
                detail_scope = {
                    "kind": "month_window" if detailed_product_model_sources else "latest_day",
                    "month": requested_month if detailed_product_model_sources else "",
                    "dates": detail_days,
        ...
        load_model = cache_key[2] if cache_key[2] and (
            cache_key[3] != "sources" or not cache_key[1]
        ) else ""
        load_options = {**load_scope, "question": cache_key[0]}
        if not cache_key[0]:
            load_options["exclude_questions"] = ENTERPRISE_MONITOR_QUESTIONS
        if load_model:
        ...
        seed_brands: set[str] = set()
        if cache_key[0] and cache_key[3] in {"brands", "brand-trends"}:
            try:
                # Scoped loads are intentionally small for fast first paint, but
                # their matcher still needs the verified brand vocabulary from
                # earlier days. Otherwise today's ai_pending answers appear to
                # contain only whichever brand happens to be globally curated.
                seed_brands = monitor_database.source_intersection_product_brands(
                    question=cache_key[0]
                )
            except Exception as exc:
                print(f"analytics brand vocabulary fallback: {exc}")
...
def _build_unified_analytics_from_snapshot(
    ...
    if len(cache_key) > 4 and cache_key[4]:
        result["filters"] = {
            **dict(result.get("filters") or {}),
            "summary_month": cache_key[4],
        }
...
def _unified_analytics_without_coalescing(params):
    ...
    if cache_key[3] == "sources":
        cache_key = (*cache_key, (params.get("summary_month") or [""])[0])
...
def _build_source_intersection_payload(question="", date=""):
    ...
    """
    if not date:
        # A mature installation has more than one million citation rows.  The
        # old all-history build materialized all grouped citations at once and
        # could consume 13+ GiB.  Each intersection row already belongs to one
        # natural day, so partitioning by day preserves exact semantics while
        # bounding the peak working set to a single day.
        days = monitor_database.analytics_dates(question=question)
        combined = {
            "generated_at": datetime.now(CST).isoformat(timespec="seconds"),
            "filters": {"question": question, "date": "", "view": "source-intersections"},
            "competitor_brands": [],
            "common_owned_sources": [],
            "two_model_owned_sources": [],
            "common_competitor_sources": [],
            "two_model_competitor_sources": [],
            "common_all_competitor_sources": [],
            "two_model_all_competitor_sources": [],
        }
        competitor_brands = set()
        row_fields = tuple(key for key in combined if key.endswith("_sources"))
        for selected_day in days:
            daily = _build_source_intersection_payload(question, selected_day)
            competitor_brands.update(daily.get("competitor_brands") or [])
            for field in row_fields:
                combined[field].extend(daily.get(field) or [])
        combined["competitor_brands"] = sorted(competitor_brands)
        return combined
...
def _refresh_source_intersections(question, date, version):
    key = (question, date)
    try:
        # A single intersection build can scan close to a million source rows.
        # Different filter keys used to start one PostgreSQL sort each, which
        # exhausted both the page file and the database temp volume.  Serialize
        # these builds globally; stale cache entries remain immediately usable.
        with _SOURCE_INTERSECTION_BUILD_SEMAPHORE:
            payload = _build_source_intersection_payload(question, date)
        with _SOURCE_INTERSECTION_LOCK:
            _remember_source_intersection_locked(key, (version, payload))
...
def _source_intersection_payload(params):
    ...
    if cached:
        with _SOURCE_INTERSECTION_LOCK:
            ...
            if (
                due
                and key not in _SOURCE_INTERSECTION_BUILDING
                and len(_SOURCE_INTERSECTION_BUILDING) < _SOURCE_INTERSECTION_BACKGROUND_LIMIT
            ):
    ...
    if isinstance(persisted, dict):
        with _SOURCE_INTERSECTION_LOCK:
            ...
            _remember_source_intersection_locked(key, (version, persisted))
            _SOURCE_INTERSECTION_LAST_VALIDATED[key] = time.monotonic()
        return persisted
...
def _analytics_payload_for_client(result, include_runs=False, view="", summary_month=""):
    if view == "overview":
        result = _with_fresh_owned_product_board(result, summary_month)
    ...
    for raw_model in result.get("models") or []:
        ...
        if model.get("recent_runs"):
            cleaned_runs = []
            for raw_run in model.get("recent_runs") or []:
                run = dict(raw_run)
                raw_answer = str(run.get("answer") or "")
                clean_answer = recommendation_body(raw_answer)
                run["answer"] = clean_answer
                run["answer_raw_length"] = len(raw_answer)
                run["answer_trailing_content_removed"] = clean_answer != raw_answer.strip()
                cleaned_runs.append(run)
            model["recent_runs"] = cleaned_runs
...
def _open_remote_model_panel(model):
    ...
    script = Path(BASE_DIR) / "dashboard" / "backend" / "control_panel.py"
...
def _attach_owned_product_yesterday(board, selected_models, question, model, comparison_day):
    """Attach exact previous-calendar-day percentage-point changes to one day."""
    try:
        current_date = datetime.strptime(comparison_day, "%Y-%m-%d").date()
    except (TypeError, ValueError):
        return board
    previous_day = (current_date - timedelta(days=1)).isoformat()
    previous_key = (question, previous_day, model, "owned_product_board")
    with _OWNED_BOARD_LOCK:
        previous_cached = _OWNED_BOARD_CACHE.get(previous_key)
    previous_rows = previous_cached[1] if previous_cached is not None else _build_owned_product_board(
        previous_key, selected_models, question, previous_day, model,
    )
    previous_map = {
    ...
    for row in board:
        if str(row.get("date") or "") != comparison_day:
            continue
        previous = previous_map.get((str(row.get("question") or ""), str(row.get("product") or "")))
        for model_id in selected_models:
            status = (row.get("models") or {}).get(model_id)
            if not isinstance(status, dict) or int(status.get("eligible_runs") or 0) <= 0:
                continue
            previous_status = ((previous or {}).get("models") or {}).get(model_id)
            if not isinstance(previous_status, dict) or int(previous_status.get("eligible_runs") or 0) <= 0:
                status["previous_day_rate"] = None
                status["day_over_day_change"] = None
                continue
            previous_rate = float(previous_status.get("recommendation_rate") or 0)
            current_rate = float(status.get("recommendation_rate") or 0)
            status["previous_day_rate"] = round(previous_rate, 2)
            status["day_over_day_change"] = round(current_rate - previous_rate, 2)
            for surface_id, surface in dict(status.get("surfaces") or {}).items():
                if not isinstance(surface, dict) or int(surface.get("eligible_runs") or 0) <= 0:
                    continue
                previous_surface = dict(previous_status.get("surfaces") or {}).get(surface_id)
                if not isinstance(previous_surface, dict) or int(previous_surface.get("eligible_runs") or 0) <= 0:
                    surface["previous_day_rate"] = None
                    surface["day_over_day_change"] = None
                    continue
                previous_surface_rate = float(previous_surface.get("recommendation_rate") or 0)
                current_surface_rate = float(surface.get("recommendation_rate") or 0)
                surface["previous_day_rate"] = round(previous_surface_rate, 2)
                surface["day_over_day_change"] = round(
                    current_surface_rate - previous_surface_rate, 2,
                )
    return board


def _build_owned_product_summary(
    cache_key, selected_models, question, model, month, month_start, month_end,
):
    version = monitor_database.scope_version(cache_key)
    database_cached = monitor_database.cache_get(
        cache_key, _analytics_content_token(),
    ...
    summary = (
        list(database_cached.get("owned_product_summary") or [])
        if isinstance(database_cached, dict) else None
    )
    if summary is None:
        runs_by_model = monitor_database.load_owned_product_runs(
            day_from=month_start, day_to=month_end,
            question=question, model=model,
            exclude_questions=ENTERPRISE_MONITOR_QUESTIONS,
        )
        daily_rows = daily_owned_product_recommendations(
            runs_by_model, selected_models, question=question, day_limit=3660,
        )
        summary = owned_product_recommendation_summary(
            daily_rows, selected_models, month=month,
        )
        monitor_database.cache_put(
            cache_key, _analytics_content_token(),
            {"owned_product_summary": summary},
            expected_scope_version=int(version[1]),
        )
    with _OWNED_BOARD_LOCK:
        _OWNED_BOARD_CACHE[cache_key] = (version, summary)
    return summary
...
def _refresh_owned_product_summary(
    cache_key, selected_models, question, model, month, month_start, month_end,
):
    try:
        _build_owned_product_summary(
            cache_key, selected_models, question, model,
...
def _schedule_owned_product_summary_refresh(
    cache_key, selected_models, question, model, month, month_start, month_end,
):
    now = time.monotonic()
    with _OWNED_BOARD_LOCK:
        if cache_key in _OWNED_BOARD_BUILDING:
            return
        if now - _OWNED_BOARD_LAST_VALIDATED.get(cache_key, 0.0) < 30.0:
            return
        _OWNED_BOARD_LAST_VALIDATED[cache_key] = now
        _OWNED_BOARD_BUILDING.add(cache_key)
    threading.Thread(
        target=_refresh_owned_product_summary,
        args=(cache_key, selected_models, question, model, month, month_start, month_end),
        name="owned-product-summary-refresh",
        daemon=True,
    ).start()
...
def _with_fresh_owned_product_board(result, summary_month=""):
    ...
    try:
        ...
        board = _attach_owned_product_yesterday(
            board, selected_models, question, model, visible_days[0],
        )
        requested_month = str(summary_month or "").strip()
        if not re.fullmatch(r"\d{4}-(?:0[1-9]|1[0-2])", requested_month):
            requested_month = ""
        month_anchor = requested_month or (requested_day or visible_days[0])[:7]
        month_start_date = datetime.strptime(month_anchor + "-01", "%Y-%m-%d").date()
        if month_start_date.month == 12:
            next_month = month_start_date.replace(year=month_start_date.year + 1, month=1)
        else:
        ...
        summary_key = (question, "", model, f"owned_product_summary:{month}")
        current_version = monitor_database.scope_version(summary_key)
        with _OWNED_BOARD_LOCK:
            cached_summary = _OWNED_BOARD_CACHE.get(summary_key)
        if cached_summary is None:
            summary = _build_owned_product_summary(
                summary_key, selected_models, question, model,
                month, month_start, month_end,
            )
        else:
            summary = cached_summary[1]
            if cached_summary[0] != current_version:
                _schedule_owned_product_summary_refresh(
                    summary_key, selected_models, question, model,
                    month, month_start, month_end,
                )
        ...
        if wenxin_start:
            performance_start = max(month_start, wenxin_start)
            if performance_start <= month_end:
                performance_key = (
                    question, performance_start, "wenxin",
                    f"employee_performance_summary:{month}",
                )
                performance_model_rows["wenxin"] = _build_owned_product_summary(
                    performance_key, ["wenxin"], question, "wenxin",
                    month, performance_start, month_end,
                )
            else:
                performance_model_rows["wenxin"] = []
        return {
            **result,
            "owned_product_daily": board,
            "employee_performance": daily_employee_performance(
                summary, date=month, model_rows=performance_model_rows,
            ),
            "owned_product_summary": summary,
            "owned_product_summary_month": month,
            "owned_product_summary_months": sorted({
                str(day)[:7] for day in (result.get("dates") or [])
                if re.fullmatch(r"\d{4}-\d{2}-\d{2}", str(day))
            }, reverse=True),
            "filters": {**dict(result.get("filters") or {}), "summary_month": month},
        }
    except Exception:
        ...
        if "board" in locals():
            fallback["owned_product_daily"] = board
        if "month" in locals():
            fallback["owned_product_summary_month"] = month
            fallback["owned_product_summary_months"] = sorted({
                str(day)[:7] for day in (result.get("dates") or [])
                if re.fullmatch(r"\d{4}-\d{2}-\d{2}", str(day))
            }, reverse=True)
            fallback["filters"] = {
                **dict(result.get("filters") or {}), "summary_month": month,
            }
        return fallback
...
def is_invalid_brand_candidate(brand):
    ...
    if not text:
        return True
    # Keep ingestion/product review and dashboard analytics on the same brand
    # validity contract.  Previously a descriptor could be rejected at render
    # time yet remain stored as a historical seed and reappear on later days.
    if not analytics_valid_brand(text):
        return True
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/v1/captures":
            ...
            return
        if re.fullmatch(r"/api/v1/models/(deepseek|yuanbao|wenxin)/results", path):
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/api/analytics/source-intersections"):
            try:
                ...
                self.send_json({"ok": False, "error": str(exc)}, 500)
            return
        if self.path.startswith("/api/enterprise-analytics"):
            try:
                params = parse_qs(urlparse(self.path).query)
                self.send_cached_json(
                    self.request_cache_key("enterprise-analytics"),
                    lambda: _enterprise_analytics(params), ttl=3.0, stale_ttl=20.0,
                )
            except Exception as exc:
                self.send_json({"ok": False, "error": str(exc)}, 500)
            return
...
def main():
    ...
    def enforce_database_retention():
        retention_days = max(
            1, min(3650, int(os.environ.get("MONITOR_DATABASE_RETENTION_DAYS", "60") or 60))
        )
        while True:
            try:
                result = monitor_database.prune_old_data(retention_days)
                if result.get("runs_deleted") or result.get("ingest_events_deleted"):
                    print("Database retention applied:", result)
            except Exception as exc:
                print("Database retention failed:", exc)
            time.sleep(6 * 60 * 60)

    threading.Thread(
        target=enforce_database_retention,
    ...
    def keep_dashboard_views_warm():
        """Prebuild only bounded current-day views once per Beijing day."""
        warmed_day = ""
    ...
    def keep_source_filter_caches_warm():
        """Warm bounded current-day source pages; intersections stay on demand."""
        time.sleep(90)
        while True:
            try:
            ...
            for question in questions:
                try:
                    current_day = datetime.now(CST).date().isoformat()
                    _unified_analytics({
                        "question": [question], "date": [current_day], "view": ["sources"],
                    })
                except Exception as exc:
