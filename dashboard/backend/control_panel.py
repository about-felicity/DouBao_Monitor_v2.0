...
def build_worker_command(
    ...
    command = [
        ...
        str(ROOT / "transport" / "remote_worker.py"),
        "--model",
        model,
        "--rounds",
        str(max(1, rounds)),
...
class RemoteModelPanel:
    ...
    def tasks(self) -> int:
        if self.model != "wenxin":
            return 1
        # Baidu rate-limits concurrent automated searches from the same host.
        # Keep the UI and the effective worker count aligned with stable mode.
        self.tasks_var.set("1")
        return 1
    ...
    def build_ui(self) -> None:
        ...
        if self.model == "wenxin":
            ttk.Label(settings, text="采集模式（稳定单任务）").grid(
    ...
    def check_account(self) -> None:
        if self.account_check_running or (self.process and self.process.poll() is None):
            return
        if self.external_worker_running or remote_collector_running(self.model):
            self.external_worker_running = True
            self.account_verified = True
            self.status_var.set("运行中（已有采集任务）")
            self.account_var.set(
                "文心采集器正在运行；为避免占用生产浏览器档案，已跳过重复百度检测"
                if self.model == "wenxin"
                else "采集器正在运行；已跳过重复登录检测"
            )
            self.account_button.configure(state="normal")
            self.start_button.configure(state="disabled")
            if hasattr(self, "stop_button"):
                self.stop_button.configure(state="disabled")
            self.append_log("检测到现有采集任务，未启动第二个浏览器检测进程。")
            return
